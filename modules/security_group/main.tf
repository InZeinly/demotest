...
# Creating a security group named test_jenkins_sg
# Remember, this security group is for our Jenkins EC2 instance
resource "aws_security_group" "test_jenkins_sg" {
  # Name, Description and the VPC of the Security Group
  name = "test_jenkins_sg"
  ...
  # We are setting the Name tag to test_jenkins_sg
  tags = {
    Name = "test_jenkins_sg"
  }
}